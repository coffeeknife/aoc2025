...
fn pt2(input: &Vec<i32>) -> u32 {
    ...
    for turn in input {
        ...
        if last_pos != 0 && position == 0 {
            zero_count += (turn.abs() / 100) as u32 + 1
        } else if last_pos == 0 {
            zero_count += (turn.abs() / 100) as u32
        } else if pos_new != position {
            zero_count += ((pos_new - position).abs() / 100) as u32
        }
    }
    zero_count
}
...
fn parse_input(filename: &str) -> Vec<i32> {
    ...
    BufReader::new(file)
        ...
        .map(|x| -> i32 {
            ...
            let mut line = binding.chars();
            let dir: i32 = match line.next().unwrap() {
                'L' => -1,
                'R' => 1,
                _ => panic!(),
            };
            line.flat_map(|y| y.to_digit(10))
                .fold(0, |acc, elem| acc * 10 + (elem as i32))
                * dir
        })
        .collect()
}
