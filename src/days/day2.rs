...
fn build_ids(start: u128, end: u128) -> Vec<u128> {
    ...
    for y in (x..(end.ilog10() as u128 + 1)).filter(|&x| x % 2 != 0) {
        // odd powers of 10 = equal #s of digits
        let z = y / 2;
        let mut values: Vec<u128> = (10_u128.pow(z as u32)..(10_u128.pow(z as u32 + 1)))
            .map(|x| x * 10_u128.pow(z as u32 + 1) + x)
...
fn build_ids_2(start: u128, end: u128) -> Vec<u128> {
    ...
    for y in x..(end.ilog10() as u128 + 1) {
        for z in (0..y).filter(|a| (y + 1) % (a + 1) == 0) {
            // iterate thru smaller powers of 10 that divide cleanly
            let mut values: Vec<u128> = (10_u128.pow(z as u32)..10_u128.pow(z as u32 + 1))
                .map(|x| {
                    let mut finished = x;
